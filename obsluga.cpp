    ...
    switch(ready)
            {
                ...
                default:
                {
                    ...
                    if( icmphdr->type == ICMP_TIME_EXCEEDED){            
                        ...
                        if( pid != icmphdr->un.echo.id ) { return -1;} //to nie mój pakiet
                        int sq = icmphdr->un.echo.sequence;
                        if( sq < ttl ) break; //ignorowanie pakietów z mniejszym ttl
                        
                        
                        if(timeEnd[sq].tv_sec == 0 && timeEnd[sq].tv_usec == 0)
                            std::cout << sender_ip_str << " ";
                    ...
                    }else return -1; //to nie był mój komunikat
